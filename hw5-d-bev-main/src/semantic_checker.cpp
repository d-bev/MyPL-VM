...
void SemanticChecker::visit(ForStmt &s)
{
  ...
  // check if loop condition types are compatible
  if ((type1.type_name != type2.type_name) || (type1.is_array != type2.is_array))
  {
    error("ForStmt: condition types need to be of the same type");
  }

  if (type1.type_name != "int" || type2.type_name != "int")
  {
    error("ForStmt: types need to be of type int");
  }

  symbol_table.add(s.var_decl.var_def.var_name.lexeme(), curr_type);
...
void SemanticChecker::visit(CallExpr &e)
{
  ...
  else // isn't a built-in function
  {
    ...
    for (int i = 0; i < e.args.size(); ++i)
    {
      ...
      DataType arg_type = curr_type;
      DataType param_type = fun_defs[fun_name].return_type;

      if (curr_type.type_name == "void")
      {
        // skip if curr type is void
      }
      else if ( // param_type == curr_type
          (param_type.type_name == curr_type.type_name) &&
          (param_type.is_array == curr_type.is_array))
      {
        // skip if types are equivalent
      }
      else
      {
        error("CallExpr: typing error");
      }
    }
